...
router.get("/", async (req, res) => {
    ...
    if (search) {
        const term = `%${search}%`;
        conditions.push(or(like(examPackage.title, term), like(examPackage.description, term)));
    }
    if (type) {
        conditions.push(eq(examPackage.type, type));
    }
    if (educationLevel) {
        conditions.push(eq(examPackage.educationLevel, educationLevel));
    }
