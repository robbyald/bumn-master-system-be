...
router.get("/", async (req, res) => {
    ...
    if (role) {
        conditions.push(sql `coalesce(${userProfile.role}, 'user') = ${role}`);
    }
    if (status) {
        conditions.push(sql `coalesce(${userProfile.status}, 'active') = ${status}`);
    }
...
router.get("/:id/points", async (req, res) => {
    ...
    const conditions = [eq(userPointHistory.userId, id)];
