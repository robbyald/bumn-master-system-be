...
const spendPointsOnceForOrder = async (args) => {
    ...
    const existing = await db
        ...
        .where(and(eq(userPointHistory.userId, args.userId), eq(userPointHistory.type, "spend"), eq(userPointHistory.description, description)))
        .limit(1);
    if (existing.length > 0)
        return;
    await db
...
const formatPartnerServiceId = (raw) => {
...
const normalizeMode = (mode) => String(mode || "").toLowerCase() === "production" ? "production" : "sandbox";
const getGatewayMode = async () => {
    const rows = await db
...
const buildCustomerNoFromConfig = (raw) => {
...
const inferPaymentMethod = (raw) => {
    const checkoutName = raw?._checkoutMeta?.paymentMethodName;
    if (typeof checkoutName === "string" && checkoutName.trim())
        return checkoutName.trim();
...
const pickMethodConfigByMode = (mode, method) => {
    if (mode === "production") {
        return {
...
const normalizeVaNumber = (value) => String(value || "").replace(/\s+/g, "");
const resolveNotificationStatus = (payload) => {
    const fromGeneral = mapDokuStatus(payload);
    if (fromGeneral !== "pending")
        return fromGeneral;
    const fromVa = mapDokuVaStatus(payload);
    if (fromVa !== "pending")
        return fromVa;
    const paidAmountRaw = payload?.paidAmount?.value ??
...
const buildDefaultNotificationAck = (payload) => ({
...
const applyAckCompat2500 = (ackPayload, payload) => {
...
export const handleDokuVaNotification = async (req, res) => {
...
export const handleDokuTokenRequest = async (req, res) => {
    const meta = {
        at: new Date().toISOString(),
        method: req?.method,
        path: req?.originalUrl || req?.url,
...
router.post("/doku/checkout", async (req, res) => {
    ...
    const selectedMethod = activeMethods.find((m) => m.id === parsed.data.paymentMethodId) || activeMethods[0];
    ...
    try {
        ...
        const vaNumber = response?.virtualAccountNo ||
            vaData?.virtualAccountNo ||
            `${partnerServiceId}${customerNo}`;
        const vaExpired = response?.expiredDate ||
            vaData?.expiredDate ||
            expiredDate;
...
router.get("/orders/:id/status", async (req, res) => {
    ...
    let statusCheckMode = "none";
    let statusCheckRequest = null;
...
router.get("/orders/:id/how-to-pay", async (req, res) => {
    ...
    const howToPayApi = raw?.virtualAccountData?.additionalInfo?.howToPayApi ||
