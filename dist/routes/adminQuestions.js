...
const buildInvalid = (issues, explanation) => ({
    is_valid: false,
    verdict: "invalid",
    issues,
    explanation
});
const extractInts = (text) => (text.match(/\d+/g) || []).map((n) => Number(n));
const ruleBasedValidateVlr = (input) => {
    ...
    const forbiddenContextPhrases = /(oleh karena itu|dengan demikian|dapat disimpulkan|kesimpulannya|sehingga dapat)/i;
    if (forbiddenContextPhrases.test(ctx)) {
        return buildInvalid([{ code: "SPOILER_IN_CONTEXT", message: "Context mengandung kata kesimpulan." }], "Context harus murni data/fakta. Ditemukan kata kesimpulan dalam context_text.");
    }
    ...
    if (stmtNorm && ctxNorm.includes(stmtNorm)) {
        return buildInvalid([{ code: "SPOILER_IN_CONTEXT", message: "Statement muncul di dalam context." }], "Statement tidak boleh diulang di dalam context_text karena menjadi spoiler.");
    }
    ...
    if (hasAllStatement && hasRemainderGroup && !remainderHasStatus) {
        return buildInvalid([{ code: "INCOMPLETE_CATEGORY", message: "Kategori sisa/lainnya tidak memiliki status atribut." }], "Statement memakai kata 'semua', tetapi context menyebut kelompok sisa tanpa status atribut. Kesimpulan tidak bisa dipastikan.");
    }
    ...
    if (explLower.includes("dalam konteks soal ini") || explLower.includes("diasumsikan") || explLower.includes("dianggap")) {
        return buildInvalid([{ code: "HALLUCINATION", message: "Explanation menambah asumsi di luar context." }], "Explanation menggunakan asumsi di luar teks (kata seperti 'diasumsikan' / 'dalam konteks soal ini').");
    }
    ...
    const opCue = /[=+\-*/]/;
    const hasMathEvidence = (mathCue.test(stmt) || mathCue.test(explanation) || opCue.test(explanation)) &&
        stmtNums.length > 0 &&
        explNums.length > 0;
    ...
    if (hasMathEvidence && lastExplNum !== null) {
        if (correctAnswer === "A" && stmtNums[0] !== lastExplNum) {
            return buildInvalid([{ code: "WRONG_KEY", message: "Hasil hitung tidak sama dengan angka pada statement, tetapi jawaban A." }], "Explanation menghitung nilai yang berbeda dari statement, namun kunci jawaban menyatakan benar.");
        }
        if (correctAnswer === "B" && stmtNums[0] === lastExplNum) {
            return buildInvalid([{ code: "WRONG_KEY", message: "Hasil hitung sama dengan statement, tetapi jawaban B." }], "Explanation mendukung statement, tetapi kunci jawaban menyatakan salah.");
        }
    }
    if (explLower.includes("simpulan adalah benar") && correctAnswer !== "A") {
        return buildInvalid([{ code: "WRONG_KEY", message: "Explanation menyebut benar, tetapi correctAnswer bukan A." }], "Penjelasan menyebut simpulan benar, namun correctAnswer tidak konsisten.");
    }
    if (explLower.includes("simpulan adalah salah") && correctAnswer !== "B") {
        return buildInvalid([{ code: "WRONG_KEY", message: "Explanation menyebut salah, tetapi correctAnswer bukan B." }], "Penjelasan menyebut simpulan salah, namun correctAnswer tidak konsisten.");
    }
    if (explLower.includes("tidak dapat disimpulkan") && correctAnswer !== "C") {
        return buildInvalid([{ code: "WRONG_KEY", message: "Explanation menyebut tidak dapat disimpulkan, tetapi correctAnswer bukan C." }], "Penjelasan menyebut tidak dapat disimpulkan, namun correctAnswer tidak konsisten.");
    }
    const headMatches = Array.from(ctx.matchAll(/(\d+)\s+([A-Za-zÀ-ÿ]+)\s+[A-Za-zÀ-ÿ]+/g));
...
const blueprints = {
    "Number Sequence": readFileSync(resolve("prompts/tkd-number-sequence-blueprint.md"), "utf-8"),
    "Word Classification": readFileSync(resolve("prompts/tkd-word-classification-blueprint.md"), "utf-8"),
    "Verbal Logical Reasoning": readFileSync(resolve("prompts/tkd-verbal-blueprint.md"), "utf-8"),
...
const buildSharedContextDetail = (ctx) => {
...
const deriveOcrContextTitle = (question) => {
...
const buildOcrGroupedDetail = (title) => {
...
const ensureOcrSourceDetailGroup = (sourceDetail, question) => {
    const raw = String(sourceDetail || "").trim();
    if (!raw)
        return raw;
    if (/GRP:[A-Z0-9]+/i.test(raw))
        return raw;
...
const appendImageMeta = (sourceDetail, imageUrl, imagePosition = "bottom") => {
...
const appendImageToQuestion = (question, imageUrl) => {
    const clean = String(question || "").replace(/\n{2,}\[IMAGE\]\s+\S+\s*$/i, "").trim();
    if (!imageUrl)
        return clean;
    return clean;
};
const prettifyOcrQuestion = (raw) => {
...
const extractOptionsFromQuestionText = (rawQuestion) => {
...
router.post("/", async (req, res) => {
    ...
    const { category, subcategory, difficulty, usageType, question, options, correctAnswer, explanation, source, sourceDetail, imageUrl, imagePosition, sharedContext, } = parsed.data;
...
router.post("/enhance-image", async (req, res) => {
    ...
    try {
        ...
        const runEdit = async (model) => {
            const form = new FormData();
            form.append("model", model);
            form.append("prompt", "Perjelas gambar soal agar teks/angka lebih tajam dan terbaca. Pertahankan isi, layout, urutan angka, opsi, dan struktur asli. Jangan menambah atau menghapus elemen.");
        ...
        for (const item of parsed.items || []) {
            ...
            if ((!hasValidAnswer || !explanation) && question && options.length >= 2) {
                ...
                if (solveRes.ok) {
                    ...
                    if (solveContent) {
                        const solved = JSON.parse(solveContent);
...
router.post("/validate", async (req, res) => {
    ...
    const ruleBased = payload.subcategory === "Verbal Logical Reasoning"
...
router.post("/generate", async (req, res) => {
    ...
    const examples = subcategory === "Verbal Logical Reasoning"
        ...
        : await db
            ...
            .from(questionBank)
            .where(and(eq(questionBank.category, category), eq(questionBank.subcategory, subcategory), eq(questionBank.status, "approved")))
            .orderBy(sql `random()`)
            .limit(3);
    ...
    if (subcategory === "Adaptif") {
        userLines.push("Perhatian: Hindari jawaban benar yang berfokus pada 'mengajari rekan kerja' atau 'mengadakan pelatihan' (itu masuk Kompeten).", "Untuk Adaptif, fokuskan jawaban benar pada inisiatif pribadi, penyesuaian proses, dan perbaikan sistem/teknologi.");
    }
    if (subcategory === "Verbal Logical Reasoning") {
        const targetPattern = pickVlrPattern();
        userLines.push("", `Pola soal wajib: ${targetPattern}.`, "Jika pola wajib bukan kuantitatif, DILARANG menggunakan data hitung/himpunan sebagai inti soal.", "Pastikan pola soal berbeda dari dua contoh teratas.");
        if (sharedContext && (contextText || "").trim()) {
            const fixedContext = (contextText || "").trim();
            userLines.push("", "MODE KHUSUS: SHARED CONTEXT", "Gunakan context_text berikut apa adanya (jangan ubah fakta, angka, maupun entitas):", fixedContext, "", "Buat statement_to_judge baru yang berbeda dari statement sebelumnya.", "statement_to_judge wajib bisa dinilai dengan opsi A/B/C (bukan opini).");
            if (Array.isArray(usedStatements) && usedStatements.length > 0) {
                userLines.push("Dilarang mengulang statement berikut:", ...usedStatements.map((s, i) => `${i + 1}. ${s}`));
            }
        }
    }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        const validationError = (() => {
            if (Array.isArray(parsedOut?.options)) {
                parsedOut.options = parsedOut.options.map((opt) => opt.replace(/^[A-E]\.?\s*/i, "").trim());
            }
            if (isVlr) {
                ...
                stmt = stmt.replace(/^(Simpulan bahwa\s+|Oleh karena itu,\s+|Dengan demikian,\s+|Maka,\s+|Sehingga,\s+)/i, "").trim();
            ...
            const truncated = optionStart >= 0 ? rawQuestion.slice(0, optionStart).trim() : rawQuestion.trim();
            if (parsedOut) {
    ...
    const ruleBased = validate && subcategory === "Verbal Logical Reasoning"
        ? ruleBasedValidateVlr({
    ...
    const validation = ruleBased
        ...
        : validate
            ...
            : null;
    const aiValid = validation ? validation.is_valid && validation.verdict === "valid" : true;
    const generatedSourceDetail = subcategory === "Verbal Logical Reasoning" &&
        sharedContext &&
...
router.get("/", async (req, res) => {
    ...
    if (q) {
        const pattern = `%${q}%`;
        whereParts.push(or(like(questionBank.id, pattern), like(questionBank.question, pattern)));
    }
    if (category)
        whereParts.push(eq(questionBank.category, category));
    if (subcategory)
        whereParts.push(eq(questionBank.subcategory, subcategory));
    if (difficulty)
        whereParts.push(eq(questionBank.difficulty, difficulty));
    if (status)
        whereParts.push(eq(questionBank.status, status));
    if (source)
        whereParts.push(eq(questionBank.source, source));
    if (sourceDetail)
        whereParts.push(eq(questionBank.sourceDetail, sourceDetail));
    if (sourceDetailPrefix)
        whereParts.push(like(questionBank.sourceDetail, `${sourceDetailPrefix}%`));
    ...
    const totalRowsQuery = await db
        .select({ total: sql `count(${questionBank.id})` })
        .from(questionBank)
        .leftJoin(questionUsage, eq(questionUsage.questionId, questionBank.id))
        .where(whereClause)
        .groupBy(questionBank.id)
    ...
    const statsTotal = await db.select({ total: sql `count(*)` }).from(questionBank);
    const statsUsed = await db.select({ total: sql `count(*)` }).from(questionUsage);
    const statsDraft = await db.select({ total: sql `count(*)` }).from(questionBank).where(eq(questionBank.status, "draft"));
    const statsApproved = await db.select({ total: sql `count(*)` }).from(questionBank).where(eq(questionBank.status, "approved"));
    const statsArchived = await db.select({ total: sql `count(*)` }).from(questionBank).where(eq(questionBank.status, "archived"));
    return res.json({
        items: rows,
        total: totalRows || 0,
        page,
