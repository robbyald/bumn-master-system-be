...
const router = Router();
const ALLOWED_STATUS = ["pending", "paid", "expired", "failed", "cancelled"];
...
router.get("/", async (req, res) => {
    ...
    const whereClause = and(status !== "all" && ALLOWED_STATUS.includes(status)
        ? eq(paymentOrder.status, status)
        : undefined, q
        ? or(like(paymentOrder.externalOrderNo, `%${q}%`), like(paymentOrder.externalInvoiceNo, `%${q}%`), like(user.name, `%${q}%`), like(user.email, `%${q}%`), like(examPackage.title, `%${q}%`))
        : undefined);
...
router.patch("/:id/status", async (req, res) => {
    ...
    if (!id || !ALLOWED_STATUS.includes(nextStatus)) {
