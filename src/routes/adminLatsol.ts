...
router.get("/sets", async (req, res) => {
  ...
  if (categoryId) conditions.push(eq(latsolSet.categoryId, categoryId) as any);
  if (category) conditions.push(eq(latsolSet.category, category) as any);
  if (subcategory) conditions.push(eq(latsolSet.subcategory, subcategory) as any);
  if (q) conditions.push(like(latsolSet.title, `%${q}%`) as any);

  const whereClause = conditions.length ? and(...conditions) : undefined;

  const totalRows = await db
    .select({ count: sql<number>`count(${latsolSet.id})` })
    .from(latsolSet)
  ...
  const rows = await db
    ...
    questionCount: sql<number>`count(${latsolSetQuestion.id})`,
  })
    .from(latsolSet)
...
router.post("/sets", async (req, res) => {
  ...
  const id = randomUUID();
  const featureCode =
    parsed.data.featureCode && parsed.data.featureCode.trim()
...
router.post("/sets/:id/autofill", async (req, res) => {
  ...
  const candidates = await db
    ...
    .where(
      and(
        eq(questionBank.category, set.category),
        eq(questionBank.subcategory, set.subcategory),
        eq(questionBank.status, "approved"),
        sql`${latsolSetQuestion.questionId} IS NULL`
      ) as any
    )
    .orderBy(sql`RANDOM()`)
